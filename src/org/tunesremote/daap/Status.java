			...
			while (true) {
				...
				while (playStatus == STATE_PLAYING) {
					...
					try {
						...
						// changed
						Log.d(TAG,
								"someone jolted us during STATE_PLAYING loop");
						continue;
					}
			...
			ThreadExecutor.runTask(new Runnable() {
				public void run() {
					try {
						...
						coverCache = RequestHelper.requestBitmap(String
								.format("%s/ctrl-int/1/nowplayingartwork?mw=320&mh=320&session-id=%s",
